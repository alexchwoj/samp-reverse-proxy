...
class QueryCloner:
	...
	def start(self):
		...
		print(f"{Fore.GREEN}[+]{Fore.WHITE} Listening on {self.bind_address}:{self.bind_port}, cloning {self.target_address}:{self.target_port}, self: {self}")
...
if __name__ == '__main__':
	print(f'{Fore.CYAN}Hyaxe - Query cloner{Fore.WHITE}')
     
	server = QueryCloner('0.0.0.0', 7777, '45.134.11.133', 7777)
	server.start()
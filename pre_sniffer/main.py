			...
			if proto == 17:
				...
				if len(values) == 47:
					if not conn_address in shared_validated:
						print(F"[Client join] Registered player in port {dest_port} > {conn_address} (len: {len(values)})")
						...
						try:
							req = requests.post(
								f'{validator_url}/proxy/register/player',
								headers = {
							...
							print(f'[Validator] Validated player {conn_address}, response: {req.json()}')
